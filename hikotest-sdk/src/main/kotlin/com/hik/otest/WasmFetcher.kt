...
/** One fetched release: the wasm binary plus its manifest.json (best-effort). */
internal data class FetchedBundle(val wasmBytes: ByteArray, val manifestJson: String?)
...
internal class WasmFetcher(context: Context, private val config: HikotestConfig) {
    ...
    suspend fun getBundle(): FetchedBundle = withContext(Dispatchers.IO) {
        ...
        if (release != null) {
            ...
            if (!wasmFile.exists() || release.tag != cachedTag) {
                ...
                persist(bytes, downloadManifest(release), release.tag)
            }
        }
        ...
        FetchedBundle(wasmFile.readBytes(), cachedManifest())
    }
    ...
    suspend fun checkForUpdate(): FetchedBundle? = withContext(Dispatchers.IO) {
        ...
        persist(bytes, manifestJson, release.tag)
        FetchedBundle(bytes, manifestJson)
    }
    ...
    private fun persist(bytes: ByteArray, manifestJson: String?, tag: String) {
    ...
    private data class ReleaseInfo(val tag: String, val wasmAssetId: Long, val manifestAssetId: Long?)
    ...
    private fun fetchLatestRelease(): ReleaseInfo? {
        ...
        return client.newCall(request).execute().use { response ->
            ...
            wasmAssetId?.let { ReleaseInfo(tag, it, manifestAssetId) }
        }
    }
